...
const teamColors: Record<string, string> = {
  ...
  'CBJ': '#002654', 'SJS': '#006D75', 'UTA': '#6CACE4',
};

export default function GoalscorerTable() {
  const [propsData, setPropsData] = useState<{ props: PlayerProp[] } | null>(null);
  ...
  let filteredProps = filterTeam === 'all' ? props : props.filter(p => p.teamAbbrev === filterTeam);
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold"
                  style={{ backgroundColor: teamColors[prop.teamAbbrev] || '#374151', color: 'white' }}>
                  {prop.teamAbbrev}
                </div>
                <div>
              ...
              <div className={`px-4 py-2 rounded-lg border ${
                prop.odds && prop.odds < 0 ? 'border-emerald-500 bg-emerald-500/10' : 'border-slate-700 bg-slate-800/50'
              }`}>
                <span className={`text-lg font-bold ${prop.odds && prop.odds > 0 ? 'text-emerald-400' : 'text-blue-400'}`}>
                  {formatOdds(prop.odds)}
                </span>
              </div>
            </div>
            <div className="flex items-center justify-between text-sm">
              <div className="text-slate-500">vs {prop.opponent} • {prop.gameTime}</div>
              <div className="text-slate-500">{prop.bookmaker}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">
              ...
              Negative odds indicate favorites to score.
            </p>
          </div>
        </div>
      </div>
