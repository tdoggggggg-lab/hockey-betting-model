...
const propTypeConfig: Record<string, { label: string; valueLabel: string; defaultLine: number; unit: string }> = {
  'goalscorer': { label: 'Goalscorer', valueLabel: 'Exp. Goals', defaultLine: 0.5, unit: 'Goals' },
  'shots': { label: 'Shots on Goal', valueLabel: 'Exp. Shots', defaultLine: 2.5, unit: 'Shots' },
  'assists': { label: 'Assists', valueLabel: 'Exp. Assists', defaultLine: 0.5, unit: 'Assists' },
  'points': { label: 'Points', valueLabel: 'Exp. Points', defaultLine: 0.5, unit: 'Points' },
  'saves': { label: 'Goalie Saves', valueLabel: 'Exp. Saves', defaultLine: 25.5, unit: 'Saves' },
};
...
export default function PlayerPropsTable({ propType }: PlayerPropsTableProps) {
  ...
  const formatBookLine = (pred: PropPrediction) => {
    // If we have a book line from API, use it
    if (pred.bookLine && pred.bookLine !== '-') {
      return pred.bookLine;
    }
    // Otherwise format with default
    return `${pred.line || config.defaultLine} ${config.unit}`;
  };
    ...
    <div className="bg-slate-900/50 rounded-xl p-6">
      ...
      <div className="mt-6 p-4 bg-slate-800/30 rounded-lg">
        ...
        <div className="text-xs text-slate-500 space-y-1">
          ...
          <p><strong>Book Line:</strong> The line from sportsbooks (e.g., &quot;{config.defaultLine} {config.unit}&quot;)</p>
